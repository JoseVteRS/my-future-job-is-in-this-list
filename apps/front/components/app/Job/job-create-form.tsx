...
const job_create = gql`
  mutation job_create($input: JobCreateInput!) {
    job_create(input: $input)
  }
`;
...
const JobCreateForm = () => {
  const [jobCreateMutation] = useMutation(job_create, {
    update(cache, { data: { job_create } }) {
      const { job_list_all: jobListAll } = cache.readQuery({
        query: job_list_all,
      });
      cache.writeQuery({
        query: job_list_all,
        data: {
          job_list_all: [...jobListAll, job_create],
        },
      });
    },
  });
  ...
  const jobCreate = async (data) => {
    const { title, url, description, extraInformation, status } = data;

    try {
      const { data } = await jobCreateMutation({
        variables: {
          input: {
            title,
            url,
      ...
      console.log('JOB_ERROR_ON_CREATE', error.message);
    }
  };
